...
const xl_utils = require('../xlfiles').PaupdfFormReport;
const ensurePath = require('../folderPath');

const PauProcess = data => {
  data.map(pau => {
    createPauReport(pau);
  });
  ...
  const fullfilePath = path.resolve(
    __dirname,
    `../../public/xlfiles/pau_pdfForm_Report.xlsx`
  );
...
const createPauReport = data => {
  ...
  let docDefinition = {
    header: {
      columns: [
        ...
        {
          text:
            'Water Year 2018 Ground Water Use / Water Year 2019 Available Water For Pre-Existing Allocation Unit (DAU)',
          alignment: 'center'
        }
      ],
      style: 'pageHeader'
  ...
  docDefinition.content.push({
    text:
      'RE: Water Years 2018 Ground Water Use and Water Years 2019 Available Water Report for a Pre-Existing Allocation Unit \n\n' +
      'For your convenience, the enclosed form(s) show the Water Year 2018 ground water use and the available water ' +
      'remaining in the current allocation period in a Pre-Existing Allocation Unit.\n\n'
  });

  docDefinition.content.push({
    text:
      'A PAU has been designated by the District in situations where the amount of ground water applied to more ' +
      'than one certified irrigated tract is not able to be determined due to the configuration of the regulated ' +
      'well(s) and/or the location of the flow meter(s) measuring the withdrawal of ground water from such regulated ' +
      'well(s). In a PAU, the allocation for each certified irrigated tract within the PAU has been added together ' +
      'so that there is one allocation for the PAU. The PAU number assigned by the District is listed below ' +
      'the landowner name, along with the certification numbers of the certified irrigated tracts owned by you ' +
      'within the PAU, and their corresponding legal descriptions, and, if applicable, the corresponding field ' +
      'name(s).\n\n'
  });
  ...
  let tractInfo = `PAU No: ${data.PAU_ID}\n`;
  let allocation = 0;
  ...
  // console.log('Alloc_Period_Data: ', data.Alloc_Period_Info);
  data.Alloc_Period_Info.map(info => {
    // console.log('E_Date: ', info.e_date);
    // console.log('Compare Date: ', moment('2018-09-29').toString());
  ...
  let al_edate = new Date(2019, 9, 30);
