...
const createFAReport = data => {
  console.log('FA Cert ', data.Cert_ID);
  ...
  docDefinition.content.push({
    text:
      '\n\n\nRE: Water Year 2018 Ground Water Use Certified Irrigated Tract \n\n' +
      'For your convenience, the enclosed form(s) show the Water Year 2018 ground water use on the certified ' +
      'irrigated tract identified by the certification number, legal descriptions(s) and, if applicable, the field ' +
      'name of that certification. The certification number has been assigned to that tract by the North Platte NRD' +
      ' for administrative purposes. The field name is the name for that tract that you have provided to the ' +
      'District.\n\n'
  });
  ...
  docDefinition.content.push({
    text: `\nWY 2018 Water Used (Acre-Inches per Acre): ${(
      Math.round(y[2018] * 10) /
      10 /
      data.Cert_Details.acres
    ).toFixed(1)}`,
    style: 'emphasis',
    bold: true
  });
